...
async def cmd_setpoint(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args or len(context.args) < 2:
        await context.bot.send_message(update.message.chat.id, text="Uso: /setpoint <MAC> <valor>")
        return
    mac = context.args[0]
...
async def cmd_periodo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args or len(context.args) < 2:
        await context.bot.send_message(update.message.chat.id, text="Uso: /periodo <MAC> <segundos>")
        return
    mac = context.args[0]
    valor = context.args[1]
...
async def cmd_modo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args or len(context.args) < 2 or context.args[1].upper() not in ['AUTO', 'MAN']:
        await context.bot.send_message(update.message.chat.id, text="Uso: /modo <MAC> AUTO o /modo <MAC> MAN")
        return
    mac = context.args[0]
...
async def cmd_rele(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await context.bot.send_message(update.message.chat.id, text="Uso: /rele <MAC>")
        return
    mac = context.args[0]
...
async def cmd_destello(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await context.bot.send_message(update.message.chat.id, text="Uso: /destello <MAC>")
        return
    mac = context.args[0]
